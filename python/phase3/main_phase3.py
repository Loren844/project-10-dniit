...
class Phase3Pipeline:
    ...
    def __init__(
        ...
        self.work_plane_z_robot = work_plane_z_robot
        ...
        # Dernière position connue de l'objet (persiste pendant l'occlusion par le bras)
        self._last_known_target:  np.ndarray | None = None
    ...
    def process_frame(
        ...
        # Si H disponible : pixel → robot XY direct (plus précis que T @ pose_cam)
        if self.robot_tf.H is not None:
            z_r = self.work_plane_z_robot
            poses_robot = []
            for det, p_cam in zip(detections, poses_cam):
                u, v = det.center_px
                t_r = self.robot_tf.transform_pixel(u, v, z_r)
                poses_robot.append(
                    type(p_cam)(label=p_cam.label, R_cam=p_cam.R_cam, t_cam=t_r))
        else:
        ...
        if poses_robot:
            ...
            # Mémoriser la position détectée (écrasée à chaque détection)
            self._last_known_target = t_target.copy()
            # ── Pré-approche articulaire ───────────────────────────────────────
            ...
            if (_seq_pre not in (PickPlaceState.IDLE, PickPlaceState.DONE)
                ...
                _eff_xy = t_target[:2]
            if (confirmed   # n'autorise PRE_APPROACH que sur track Kalman confirmé
                    and (self._last_target_xy is None
            ...
            # Aucun objet détecté — utiliser la dernière position connue
            # (l'objet est probablement occulté par le bras)
            coasting = [t for t in self.mot._tracks.values()
                        if t.confirmed and t.missed <= 8]
            if coasting:
                best = min(coasting, key=lambda t: np.linalg.norm(t.tracker.position))
                t_target = best.tracker.position.copy()
                self._last_known_target = t_target.copy()
            elif self._last_known_target is not None:
                t_target = self._last_known_target
            else:
                t_target = None
            ...
            if t_target is not None:
                ...
                R_cur = self._current_ee_rotation()
                self._dbg_t_target = t_target
                err = compute_error(t_cur, R_cur, t_target, self.R_desired,
                                    thr_t_mm=CONVERGE_THR_MM,
                                    thr_r_deg=CONVERGE_THR_DEG)
                self.errors.append(err)
                self.timestamps.append(time.time() - self._t0)
                if err.norm_t_mm < APPROACH_THR_MM:
                    self.state = PipelineState.APPROACH
                    self.ctrl.tune(gain=GAIN_APPROACH, gain_max=GAIN_APPROACH)
                else:
                    self.state = PipelineState.TRACKING
